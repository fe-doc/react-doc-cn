...
这部分内容来源于实现 stack reconciler 的笔记。

它是一份技术文档并假定读者对react的公共api也包括react如何划分为核心，渲染器，识别算法有很深的理解。如果你还不熟react的[源码总览](https://facebook.github.io/react/contributing/codebase-overview.html)，请首先阅读[源码总览](https://facebook.github.io/react/contributing/codebase-overview.html)。

stack reconciler 为目前所有使用 react 的产品代码提供动力。它位于[src/renderers/shared/reconciler](http://wwww.github.com/facebook/react/src/renderers/shared/reconciler)被React DOM和 React Native使用。
...
stack reconciler 有其内在的局限性比如其是同步的,不能中断当前操作,也不能将操作分块。新的Fiber识别算法还在开发当中，其架构和stack reconciler完全不同。将来，我们倾向用它取代stack reconciler，但是在那一刻到来前还有许多工作要做。
